...
module.exports = (options) => {
  const { myEmails, myDomain, sourceEmail } = options
  ...
  function generateEmailParamsFromJSON (body) {
    const { email, name, content, company } = JSON.parse(body)
    console.log(email, name, content)
    if (!(email && name && content && body)) {
      throw new Error('Missing parameters! Make sure to add parameters \'email\', \'name\', \'content\'.')
    }

    return {
      Source: sourceEmail,
      Destination: { ToAddresses: [...myEmails[company]] },
  ...
  function generateEmailParamsFromUriEncoded (body) {
    const { email, name, content, company } = getParamsFromUrl(body)
    if (!(email && name && content && company)) {
      throw new Error('Missing parameters! Make sure to add parameters \'email\', \'name\', \'content\'.')
    }
